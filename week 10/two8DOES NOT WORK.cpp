...
public:
	...
	string getName(){return(name);}
	//diplaying the memebers of the team
	void yourTeam(){
		cout<<"\nName : "<<name;
		cout<<"\nCombat Team : ";
...
int main(){
	...
	std::vector<CHARECTOR> v;
    // EACH TEAM IS CHOSING THEIR MEMEBERS
	cout<<"Choose Team a Players : \n";
		...
		// generating new vector will the list of charactres, see TEAM class
		team1.setVec(v);
		// clears vector v
		v.clear();

    //enemy is chosing thier team
	...
	TEAM team2("Team b",num);
		...
		//generating  vector with enemy characters
		team2.setVec(v);
		//we are clearing the vector v
		v.clear();
    ...
    //DISPLAYS THE STRENGHT OF THE TEAM
		for(int i=0;i<num;i++){
...
///PLAYER IS ATTACKING AND ENEMY IF DEFENDING
...
*/
		...
		/*if(each%2==0){it=sceoend1.begin();}
		else{it=first1.begin();}*/


		if(v.at(index2).getstrength()==0){
			cout<<"F";
			//
			/*if(each%2==0){v=sceoend1;}
			else{v=first1;}*/
			//
