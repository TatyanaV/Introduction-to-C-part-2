...
int main(){
	...
	for(;;){
		...
		if(each==0){index1=0;index2=0;}// intially top order player start tuornament
			...
			index1=tmp2;//index for  altering enemy as player from recent fight
			index2=tmp1;//index for  altering player as enemy from recent fight
		...
		if(v.at(index2).getstrength()==0){
			...
			if(each%2==0){teamName.push_back('b');}// add corresponding team information
