...
int main(){
	...
	integerCheck(num);
    newLine();
	std::list<CHARECTOR> lt; // to collest characters who loose they game
	std::vector<char> teamName; // will be used when 1st, 2nd and 3rd places will be displayed
	std::vector<CHARECTOR> v; // vector of charactes

	cout<<"Choose Team a Players : \n";
    TEAM team1("Team a",num);//team a
    displayChart(); // characters statistics
	...
	int each=0;// use for team in tournament, to determine which character attacks
	...
	for(;;){
		...
		if(v.at(index2).getstrength()==0){
			...
			if((firstTeam.size()+sceoendTeam.size())<=3){
			    lt.push_back(v.at(index2));// add the charactes to the looser list
			if(each%2==0){
			    teamName.push_back('b');// add information what team the characte to the team name vector
			    }// add corresponding team information
			else{
			    teamName.push_back('a');// add information what team the characte to the team name vector
			    }
			}
			// delete charector form 'v' safely
			if(v.size()==1){// if this is the last character in the list of the team
			    v.clear();
			    }/////////
            else{
				if(index2==0){ // if there are more than 1 charactes on the list of the team, only 1st character will be erased
				    v.erase(it);
                }
				else{
				    v.erase(it+index2);
		...
		for(int i=0;i<max;i++){
            ...
            // try to determine who will get 1st and 2rd place.
            //this will be done based on the health points
            //whoever has the most health points will be assinged place 1
            //this is only nessesary when there are 2 players on the winning team
            while(v.size()>0){
				it=v.begin();
