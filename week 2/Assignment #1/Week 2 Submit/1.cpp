...
*************************************************/
void MakeYourSelection(char world[ROWS][COLUMNS]);
...
alive
*************************************************/
void continueManual(int row, int column,char world[ROWS][COLUMNS]);
...
*************************************************/
void countueToNextGeneration(int generation,char world[ROWS][COLUMNS]);

/************************************************
** Function:void NextGeneration(char generation[][COLUMNS])
...
*************************************************/
void goToMenuFunction(char world[ROWS][COLUMNS]);
/************************************************
...
void MakeYourSelection(char world[ROWS][COLUMNS]){
    ...
    getline(cin,entered);
    input = integerCheck(entered, 1, 3);
    int generation =0;
    switch(input)
    {
    case 1:
       // char world[ROWS][COLUMNS];
        ...
        continueManual(row, column,world);
        Display(world,generation);
        countueToNextGeneration(generation,world);
        goToMenuFunction(world);
        break;
    case 2:
        initializeToMinusSign(world);
...
void continueManual(int row, int column,char world[ROWS][COLUMNS]){
    ...
    while(cont==true){
        do{
            ...
            getline(cin,entered);
            row = integerCheck(entered, 1, 22);
        }while(row<1||row>22);

        do{
            cout<<"COLUMN index of an alive world(1-80): ";
            getline(cin,entered);
            column = integerCheck(entered, 1, 80);
...
void continueAutomate(char generation[][COLUMNS]){
    int coutAlive;
     srand(time(0));
    for (int countAlive =0; countAlive < ALIVE_CELLS; countAlive++){
                    generation[rand()%22+1][rand()%80+1] = 'X';
    }
}
...
int CountNeighbors(char generation[][COLUMNS],int index_X,int index_Y){
    int neighbors=0;
    //checking above row
    if(((index_X-1)>=0)&&((index_Y-1)>=0)&&(generation[index_X-1][index_Y-1]=='X')){ neighbors++;}
    if(((index_X-1)>=0)&&(generation[index_X-1][index_Y]=='X')){neighbors++;}
    if(((index_X-1)>=0)&&((index_Y+1)<COLUMNS)&&(generation[index_X-1][index_Y+1]=='X')){neighbors++;}
    //checking same row
    if(((index_Y-1)>=0)&&(generation[index_X][index_Y-1]=='X')){neighbors++;}
    if(((index_Y+1)<COLUMNS)&&(generation[index_X][index_Y+1]=='X')){neighbors++;}
    //checking below row
    if(((index_X+1)<COLUMNS)&&((index_Y-1)>=0)&&(generation[index_X+1][index_Y-1]=='X')){neighbors++;}
    if(((index_X+1)<COLUMNS)&&(generation[index_X+1][index_Y]=='X')){neighbors++;}
    if(((index_X+1)<COLUMNS)&&((index_Y+1)<COLUMNS)&&(generation[index_X+1][index_Y+1]=='X')){neighbors++;}
    return neighbors;
}
...
*************************************************/
void SurvivorsAndNewborns(char generation[][COLUMNS], char nextGeneration[][COLUMNS],int neighbors,int index_X,int index_Y){
    if(neighbors>3 && generation[index_X][index_Y]=='X'){//overcrowding death
        nextGeneration[index_X][index_Y]='-';
    }
    else if(neighbors<=1 &&generation[index_X][index_Y]=='X'){//dies from lonliness
        nextGeneration[index_X][index_Y]='-';
      }
    else if(neighbors==3 &&generation[index_X][index_Y]=='-'){//birth if 3 neighbors
        nextGeneration[index_X][index_Y]='X';
    }
    else
